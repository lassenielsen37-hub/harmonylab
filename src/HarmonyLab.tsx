...
const Pot: React.FC<{ value: number; label?: string }> = ({ value, label = "Input" }) => {
  ...
  const arc = (ang: number) => ({ x: cx + r * Math.cos(ang), y: cy + r * Math.sin(ang) });
  const s = arc(startAngle);
  const e = arc(angle);
  const eBg = arc(endAngle);
  const large = v > 0.5 ? 1 : 0;

  const bgPath = `M ${s.x} ${s.y} A ${r} ${r} 0 1 1 ${eBg.x} ${eBg.y}`;
  const fgPath = `M ${s.x} ${s.y} A ${r} ${r} 0 ${large} 1 ${e.x} ${e.y}`;
...
export default function HarmonyLab() {
  ...
  useEffect(() => {
    ...
    bus.fan(Tone.getDestination(), analyser, meter);
    return () => {
